...
const {ComponentA} = require('./pages/prova.js')
...
const app = {
  ...
  data: function () {
    return {
      ...
      componenti : [],
      headers : [
        { name: 'ArolCode', label: 'ArolCode', field: 'ArolCode' },
        { name: 'Descrizione', label: 'Description', field: 'Description'},
        { name: 'Manufacturer', label: 'Manufacturer', field: 'Manufacturer'},
        { name: 'CommercialCode', label: 'CommercialCode', field: 'CommercialCode'}
      ] 
    }
  },
  components: {
...
createConnection("magazzinoEle").then(async connection => {
  VueApp.componenti2 = await connection.getRepository(codificati).find();
})
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
